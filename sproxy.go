...
type Event struct {
	...
	Request      bool      `json:"is_req,omitempty"`
	ResponseSize int64     `json:"res_size,omitempty"`
...
func (mr *MetricsReporter) Do() {
	...
	for {
		...
		go mr.reportEvents(drainedEvents, startTime, time.Now(), i)
	}
}
...
func (mr *MetricsReporter) reportEvents(events []*Event, startTime, endTime time.Time, nIter uint64) {
	nResponses := 0
	nRequests := 0
...
type InflightStats struct {
	RateResponseStats map[int]float64 `json:"rate_stats,omitempty"`

	AverageResponseDurationSeconds float64 `json:"avg_response_secs,omitempty"`

	NRequests int `json:"nreqs,omitempty"`

	RequestIDs []string `json:"ids,omitempty"`
}
